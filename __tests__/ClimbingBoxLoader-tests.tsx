...
xdescribe\("ClimbingBoxLoader", () => {
  ...
  it("should contain styles created using default props", () => {
    ...
    expect(loader.find("span span span").at(0)).toHaveStyleRule("border", `0.25em solid ${defaultColor}`);
    expect(loader.find("span span span").at(1)).toHaveStyleRule("border-left", `0.25em solid ${defaultColor}`);
  });
  ...
  it("should render the correct color based on prop", () => {
    ...
    expect(loader.find("span span span").at(0)).not.toHaveStyleRule("border", `0.25em solid ${defaultColor}`);
    expect(loader.find("span span span").at(0)).toHaveStyleRule("border", `0.25em solid ${color}`);

    expect(loader.find("span span span").at(1)).not.toHaveStyleRule("border-left", `0.25em solid ${defaultColor}`);
    expect(loader.find("span span span").at(1)).toHaveStyleRule("border-left", `0.25em solid ${color}`);
  });

  const sizeExpectStatements = (loader: ReactWrapper, length: number, unit?: string) => {
    expect(loader.find("span span")).not.toHaveStyleRule("font-size", `${defaultSize}${defaultUnit}`);
    expect(loader.find("span span")).toHaveStyleRule("font-size", `${length}${unit || defaultUnit}`);
  };
  ...
  speedMultiplierSpecs(ClimbingBoxLoader, speedMultiplierExpectStatements);
});
